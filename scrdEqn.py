...
class time_evolution:
    ...
    '''
    def __init__(self, hbar, m, quantum_number, total_time, dt, L, x, n, a, l):
        self.hbar = hbar
        self.mass = m
    ...
    def gaussian_wave_packet(self,x,x0,l,a):
        A = (1/(4*a**2))**(1/4.0)
        self.psi_x0 = A*(np.exp((-(x - x0)**2)/(4*a**2)) * np.exp(1j*l*x)).reshape(len(x),1)
        print("psi_x0: " + str(self.psi_x0.shape))


    def normalize(self):
        self.A = ( 1/(np.sqrt(np.trapz((np.conj(self.psi_x0[:,0]) * self.psi_x0[:,0]), x[:,0]))))
    ...
    def C_n(self):
        ...
        for i in range(0,quantum_number):
            self.Cn[i,0] = np.trapz((np.conj(self.phi) * self.psi_x0_normalized)[:,i], x[:,0])
    ...
    def schrodinger_equation(self, total_time):
        ...
        for j in range(0, total_time, dt):
            ...
            for k in range(0, quantum_number):
                self.psi_xt[:,0] = self.psi_xt[:,0] + (self.Cn[0,k] * self.phi[:,k] * (np.exp((-1j * self.En[0,k] * time)/hbar)))
                count += 1
...
# plot
            ...
            plt.plot(x, np.real(self.psi_xt),'r', label='real' r'$\psi(x,t)$', linewidth = 0.75)
            plt.plot(x, np.imag(self.psi_xt),'b', label=r'$imag \psi(x,t)$', linewidth = 0.75)
            plt.plot(x, np.abs(self.psi_xt),'y', label=r'$|\psi(x,t)|$', linewidth = 0.75)
...
choose_custom = int(input('Enter \'1\' to run or \'2\' to customize: '))

if choose_custom  == int(1):
    pass
elif choose_custom  == int(2):
...
choose_run= int(input('Enter \'1\' to run for loop or \'2\' to run animation for recording: '))

Schrodinger = time_evolution(hbar,m,quantum_number,total_time,dt,L,x,n,a,l)

if choose_run  == int(1):
    Schrodinger.gaussian_wave_packet(x,x0,l,a)
...
# animate
    ...
    psi_xt_real, = ax.plot([], [], c='g', label='real' r'$\psi(x,t)$', linewidth = 0.85)
    psi_xt_imag, = ax.plot([], [], c='v', label=r'$imag \psi(x,t)$', linewidth = 0.85)
    psi_xt_abs, = ax.plot([], [], c='o', label=r'$|\psi(x,t)|$', linewidth = 0.85)
    ...
    def animate(i):
        ...
        for k in range(0, quantum_number):
            psi_xt[:,0] = psi_xt[:,0] + (Schrodinger.Cn[0,k] * Schrodinger.phi[:,k] * (np.exp((-1j * Schrodinger.En[0,k] * i)/hbar)))
        ...
        record_response = str(input('Do you want to record? Enter y or n: '))

        if record_response == 'y':
            animate = matplotlib.animation.FuncAnimation(fig, animate, init_func=init, frames=3000, interval=1, repeat=False)
